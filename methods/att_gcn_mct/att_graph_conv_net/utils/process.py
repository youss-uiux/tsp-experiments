        ...
        for batch_num in progress_bar(range(batches_per_epoch), parent=master_bar):
            ...
            if mode == 'val':  # Validation: faster 'vanilla' beamsearch
                ...
                bs_nodes = beamsearch_tour_nodes_shortest(
                    y_preds, x_edges_values, beam_size, batch_size, num_nodes, dtypeFloat, dtypeLong,
                    probs_type='logits')
    ...
    for epoch in epoch_bar:
        ...
        epoch_bar.write(
            't: ' + metrics_to_str(epoch, train_time, learning_rate, train_loss, train_err_edges, train_err_tour,
                                   train_err_tsp, train_pred_tour_len, train_gt_tour_len, num_neighbors))
        ...
        if epoch % val_every == 0 or epoch == max_epochs - 1:
            ...
            epoch_bar.write(
                'v: ' + metrics_to_str(epoch, val_time, learning_rate, val_loss, val_err_edges, val_err_tour,
                                       val_err_tsp, val_pred_tour_len, val_gt_tour_len, num_neighbors))
